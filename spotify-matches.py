...
def getSpotifyTracks(url, token):
    ...
    try:
        while url:
            decodedData = getData(url, {'Authorization': 'Bearer ' + token})
...
def main():
    ...
    spotifyTracks = getSpotifyTracks(spotifyPlaylistUrl + '/tracks?fields=items(track(id,name,artists(name)),added_by(id)),next', spotifyToken)
